...
function Header() {
  ...
  return (
    <header className="fixed top-0 left-0 w-full z-[9999] bg-transparent px-4 py-6">
      <div className="flex items-center justify-between w-full">
        ...
        {/* 3️⃣ Navigation Links (Desktop only) */}
            ...
            {menuItems.map((item) => {
              ...
              return (
                <Link
                  key={item.name}
                  to={item.path}
                  className="flex flex-col items-center cursor-pointer pb-1 group"
                >
                  <div
                    className={`flex items-center space-x-1 relative w-full transition-colors duration-200 ${
                      isActive
                        ? "text-red-600"
                  ...
                  >
                    ...
                    <img
                      src={item.icon}
                      alt={`${item.name} Icon`}
                      className={`w-4 h-4 md:w-5 md:h-5 transition-filter duration-200 ${
                        isActive
                          ? "filter brightness-100 contrast-200 sepia saturate-200 hue-rotate-330"
                          : "filter brightness-0 opacity-70 group-hover:filter group-hover:brightness-100 group-hover:contrast-200 group-hover:sepia group-hover:saturate-200 group-hover:hue-rotate-330"
                      }`}
                    />
                    {isActive && (
                      <span className="absolute bottom-[-2px] left-0 w-full h-0.5 bg-red-600 transition-all duration-300 ease-in-out"></span>
                    )}
                  </div>
                </Link>
              );
      ...
      {/* 5️⃣ Mobile Dropdown Menu */}
      <div
        className={`lg:hidden ${
          isMenuOpen ? "block" : "hidden"
        } mt-4  backdrop-blur-sm p-4 rounded-lg shadow-xl`}
      >
